...
describe("SemaphoreVoting", () => {
    let semaphoreVotingContract: StarVoting
    ...
    before(async () => {
        const { semaphoreVoting, pairingAddress } = await run("deploy:star-voting", {
            logs: true
        })
    ...
    //     const vote = 1
    //     const identitySecond = new Identity("test2")
    //     const group = new Group(pollIds[1], treeDepth)

    //     group.addMembers([identity.commitment, BigInt(1), identitySecond.commitment])

    //     let fullProof: FullProof

    //     before(async () => {
    //         await semaphoreVotingContract.connect(accounts[1]).addVoter(pollIds[1], BigInt(1))
    //         const tx = await semaphoreVotingContract.connect(accounts[1]).addVoter(pollIds[1], identitySecond.commitment)
    //         const receipt = await tx.wait()
    //         const event = receipt.events?.find((e) => e.event === "MemberAdded")
    //         console.log(event)
